...
#if HAS_LCD_MENU
    if (show_lcd)
      lcd_pause_show_message(PAUSE_MESSAGE_STATUS, mode);
#endif
    return false;
  }
...
#if HAS_LCD_MENU
    if (show_lcd)
      lcd_pause_show_message(PAUSE_MESSAGE_INSERT, mode);
#endif
...
#if HAS_LCD_MENU
  if (show_lcd)
    lcd_pause_show_message(PAUSE_MESSAGE_LOAD, mode);
#endif
...
#if HAS_LCD_MENU
  if (show_lcd)
    lcd_pause_show_message(PAUSE_MESSAGE_PURGE);
#endif
...
// Pause the print job and timer
  ...
  if (!axes_need_homing())
  {
    SERIAL_ECHOLNPAIR("Park the nozzle, Axes need Homing...");
    SERIAL_EOL();
  ...
  stepper.set_directions();
#endif
  SERIAL_ECHOLNPAIR("End of pause print:", did_pause_print);
  SERIAL_EOL();
