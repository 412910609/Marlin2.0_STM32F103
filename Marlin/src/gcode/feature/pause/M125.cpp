...
void GcodeSuite::M125()
{
  ...
  // pause_print(retract, park_point, 0, show_lcd);
  SERIAL_ECHOLNPAIR("In M125, Next to pause print...");
  SERIAL_EOL();

  if (pause_print(retract, park_point, 0, show_lcd))
  {
    SERIAL_ECHOLNPAIR("Out pause, sd printing flag:", sd_printing);
    SERIAL_EOL();

    if (IS_SD_PAUSED() || show_lcd)
    {
      SERIAL_ECHOLNPAIR("Going to wait for confirmation...");
      SERIAL_EOL();
